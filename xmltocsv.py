...
# --- Jendela Dialog Pengaturan (Sama seperti versi sebelumnya, tidak ada perubahan di sini) ---
class MappingDialog(QDialog):
    # ... (Kelas MappingDialog lengkap dari jawaban sebelumnya tidak perlu diubah) ...
    # ... Cukup salin-tempel seluruh kelas MappingDialog dari jawaban terakhir Anda ke sini ...
    # (Saya akan menyingkatnya agar tidak terlalu panjang, karena tidak ada perubahan di kelas ini)
    def __init__(self, active_profile_path, parent=None):
        super().__init__(parent); self.setWindowTitle("Pengaturan Mapping & Profil Dinamis"); self.setGeometry(150, 150, 850, 700); self.active_profile_path = active_profile_path; self.config = self.load_config(self.active_profile_path); self.detected_xpaths = []; self.initUI(); self.populate_table_from_config()
    ...
    def run_detection_and_add(self):
        ...
        try:
            tree = ET.parse(xml_file); root = tree.getroot(); detected_paths = set(); self._recursive_detect(root, ".", detected_paths); self.detected_xpaths = sorted(list(detected_paths)); mapped_paths = set();
    ...
    def add_manual_row(self, default_details=None):
        ...
        combo_path = QComboBox(); combo_path.setEditable(True); combo_path.addItems(self.detected_xpaths); combo_path.setCurrentText(default_details.get("path", "")); self.table.setCellWidget(row_position, 2, combo_path)
    ...
    def save_and_accept(self):
        current_config = {"settings": {}, "columns": {}}; current_config["settings"]["item_loop_path"] = self.txt_item_loop_path.text()
        for i in range(self.table.rowCount()):
...
# --- Kelas Utama Aplikasi dengan UI Bertab ---
class XMLConverterApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Konverter Data XML <-> CSV")
        ...
        # Variabel untuk XML -> CSV
        self.xml_files = []
        self.xml_to_csv_output_file = ""

        # Variabel untuk CSV -> XML
        self.csv_file_input = ""
    ...
    def initUI(self):
        ...
        # Bagian Profil (di luar tab, berlaku untuk semua)
    ...
    def create_xml_to_csv_tab(self):
        """Menciptakan widget untuk tab XML ke CSV."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        # (Kode UI dari versi sebelumnya dipindahkan ke sini)
    ...
    def create_csv_to_xml_tab(self):
        """Menciptakan widget untuk tab CSV ke XML."""
        tab = QWidget()
        layout = QVBoxLayout(tab)
        
        input_groupbox = QGroupBox("1. Pilih File CSV Input")
        input_layout = QHBoxLayout()
        self.txt_csv_input = QLineEdit(); self.txt_csv_input.setReadOnly(True)
        btn_select_csv = QPushButton("..."); btn_select_csv.setFixedWidth(40)
        btn_select_csv.clicked.connect(self.select_csv_input)
        input_layout.addWidget(self.txt_csv_input); input_layout.addWidget(btn_select_csv)
        input_groupbox.setLayout(input_layout)

        output_groupbox = QGroupBox("2. Tentukan Folder Output untuk File XML")
        output_layout = QHBoxLayout()
        self.txt_csv_to_xml_output = QLineEdit(); self.txt_csv_to_xml_output.setReadOnly(True)
        btn_select_dir = QPushButton("..."); btn_select_dir.setFixedWidth(40)
        btn_select_dir.clicked.connect(self.select_csv_to_xml_output_dir)
        output_layout.addWidget(self.txt_csv_to_xml_output); output_layout.addWidget(btn_select_dir)
        output_groupbox.setLayout(output_layout)

        convert_groupbox = QGroupBox("3. Jalankan Konversi")
        convert_layout = QVBoxLayout()
        btn_convert = QPushButton("Konversi ke XML"); btn_convert.setStyleSheet("font-size: 16px; padding: 10px; background-color: #008CBA; color: white;")
        btn_convert.clicked.connect(self.run_csv_to_xml_conversion)
        convert_layout.addWidget(btn_convert)
        convert_groupbox.setLayout(convert_layout)

        layout.addWidget(input_groupbox); layout.addWidget(output_groupbox); layout.addWidget(convert_groupbox); layout.addStretch()
        return tab
    
    # --- Metode untuk XML -> CSV ---
    ...
    def run_xml_to_csv_conversion(self):
        ...
        try:
            ...
            self.statusBar.showMessage(f"Kesalahan: {e}"); QMessageBox.critical(self, "Error", f"Terjadi kesalahan saat konversi:\n{e}")

    # --- Metode untuk CSV -> XML ---
    ...
    def csv_to_xml_logic(self, csv_file_path, output_dir, config):
        ...
        # 1. Baca dan kelompokkan data CSV
        grouped_data = defaultdict(list)
        with open(csv_file_path, mode='r', encoding='utf-8') as f:
            reader = csv.DictReader(f)
            # Lewati baris pertama jika itu adalah tanggal konversi
            if "Conversion Date" in reader.fieldnames[0]:
                next(reader) 
            for row in reader:
        ...
        # 2. Proses setiap grup menjadi satu file XML
        file_count = 0
        for group_id, items in grouped_data.items():
            # Buat elemen root
            root_el = ET.Element(root_name)
            
            # Ambil data pertama untuk mengisi data 'root'
            ...
            # Proses setiap baris item
            for item_data in items:
                # Buat elemen item
                item_el = ET.SubElement(root_el, item_tag)
                for col_name, details in columns.items():
                    if details.get("source") == "item" and details.get("type") != "calculated":
                        self._insert_value(item_el, details, item_data.get(col_name, ''))

            # 3. Tulis pohon XML ke file
            tree = ET.ElementTree(root_el)
            ET.indent(tree, space="  ", level=0) # Untuk pretty-printing
            output_filename = os.path.join(output_dir, f"{group_id}.xml")
    ...
    def _insert_value(self, base_element, details, value):
        """Menyisipkan nilai ke dalam pohon XML berdasarkan path dari profil."""
        ...
        if col_type == "attribute":
            base_element.set(path, str(value))
        elif col_type in ["xpath", "xpath_indexed"]:
            # Buat path jika tidak ada
            current_el = base_element
            path_parts = path.strip("./").split("/")
            for i, part in enumerate(path_parts):
                # Abaikan bagian yang mengandung '[]' karena itu predikat
                if '[' in part and ']' in part:
                    # Coba temukan dengan predikat, ini implementasi sederhana
                    # Contoh: additionalPartyIdentification[additionalPartyIdentificationType='...']
                    tag_name = part.split('[')[0]
                    # Logika lebih kompleks diperlukan untuk parsing predikat penuh
                    # Untuk sekarang, kita buat saja elemennya
                    found_el = current_el.find(part)
                    if found_el is None:
                    ...
                    found_el = current_el.find(part)
                    if found_el is None:
                        found_el = ET.SubElement(current_el, part)
                    current_el = found_el
